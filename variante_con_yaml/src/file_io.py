...
def save_markdown(task_output):
    ...
    try:
        ...
        base_path = r"C:/Users/user/Desktop/CODing/crew_AI/heirerchal_process/groq_experiment/risultato"
        filename = f"{today_date}.md"
        full_path = os.path.join(base_path, filename)

        # Prepara il contenuto del file
        output_string = task_output.result() if hasattr(task_output, 'result') and callable(task_output.result) else str(task_output)
    ...
    except Exception as e:  # Cattura eccezioni più generiche legate a errori di I/O
